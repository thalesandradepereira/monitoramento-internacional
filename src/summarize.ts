...
const triageResponseSchema = z.array(z.object({
  ...
  pais: z.string().min(1).max(80),
}).strict()).max(200)

const summaryResponseSchema = z.array(z.object({
  id: z.string().min(1).max(32),
...
`
    ...
    try {
      const result = await generateContentWithRetry(
        config.gemini.models.triage,
        promptTriagem,
        z.toJSONSchema(triageResponseSchema),
      )
      const limpo = cleanGeminiJson(result.text || '[]')

      const arr = triageResponseSchema.parse(JSON.parse(limpo))
