...
# External clocks overlap the recovery window instead of waiting until all GitHub clocks have failed.
# Media wakes every 30 minutes from 03:11 through 06:41 BRT. Publisher wakes 10 minutes later,
# from 03:21 through 06:51 BRT. Application-level daily idempotency makes repeated wake-ups safe.
upsert_job \
  "$MEDIA_JOB" \
  "11,41 3-6 * * *" \
  "/dispatch/media" \
  "External failsafe mesh for Monitoramento Internacional"

upsert_job \
  "$PUBLISHER_JOB" \
  "21,51 3-6 * * *" \
  "/dispatch/publisher" \
  "External failsafe mesh for Instagram publisher"
...
echo "Media job: $MEDIA_JOB (03:11/03:41/04:11/04:41/05:11/05:41/06:11/06:41 America/Sao_Paulo)"
echo "Publisher job: $PUBLISHER_JOB (03:21/03:51/04:21/04:51/05:21/05:51/06:21/06:51 America/Sao_Paulo)"
echo
